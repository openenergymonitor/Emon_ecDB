...
var table = {
    ...
    'draw':function()
    {
        ...
        for (group in groups) 
        {
            ...
            if (table.groupshow[group]==undefined && group_num>1) table.groupshow[group]=false;
            if (table.groupshow[group]==undefined && group_num==1) table.groupshow[group]=true;
            if (table.groupshow[group]==false) {symbol = '<i class="icon-plus-sign"></i>'; visible = "display:none";}

            if (group_num>1) {
              html += "<tr><th colspan='2'><a class='MINMAX' group='"+group+"' >"+symbol+"</a> "+table.groupprefix+group.replace("_", " ")+"</th>";
